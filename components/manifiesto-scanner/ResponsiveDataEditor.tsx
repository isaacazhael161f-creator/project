/**
 * Responsive DataEditor Component
 * Provides an editable interface for manifiesto data with responsive layout
 */

import React, { useState, useEffect, useCallback } from 'react';
import { View, Text, StyleSheet, ScrollView } from 'react-native';
import { ManifiestoData, DataEditorProps, ValidationRule } from '../../types/manifiesto';
import { validateManifiestoData, sanitizeFieldValue } from '../../utils/manifiesto/validation';
...
export const ResponsiveDataEditor: React.FC<DataEditorProps> = ({
  data: initialData,
  onDataChanged,
  validationRules
}) => {
  ...
  useEffect(() => {
    const validateData = async () => {
      ...
      const errors = validateManifiestoData(state.data, validationRules);
  ...
  // Render field with responsive layout
  const renderField = (component: React.ReactNode, index: number, totalFields: number) => {
    if (isMobile) {
      return (
    ...
    const fieldsPerRow = isTablet ? 2 : 3;
    const fieldWidth = `${(100 / fieldsPerRow) - 2}%`;

    return (
      <View key={index} style={[styles.fieldContainerResponsive, { width: fieldWidth }]}>
        {component}
      </View>
    );
  };

  // Render section with responsive grid
  const renderSection = (title: string, fields: React.ReactNode[]) => (
    <View style={responsiveStyles.section}>
      <Text style={responsiveStyles.sectionTitle}>{title}</Text>
      <View style={isMobile ? styles.singleColumn : styles.multiColumn}>
        {fields.map((field, index) => renderField(field, index, fields.length))}
      </View>
    </View>
  );
  ...
  return (
    <ScrollView style={responsiveStyles.container} showsVerticalScrollIndicator={false}>
      <View style={gridStyles.container}>
        ...
        {renderSection('Información del Vuelo', [
          <DateInputField
            label="Fecha"
        ...
        {renderSection('Información de Aeronave', [
          <TextInputField
            label="Transportista"
        ...
        {renderSection('Información del Piloto', [
          <TextInputField
        ...
        {renderSection('Movimiento de Operaciones', [
          <TextInputField
        ...
        {renderSection('Información de Pasajeros', [
          <NumberInputField
            label="Nacional"
        ...
        {renderSection('Información de Carga (kg)', [
          <NumberInputField
            label="Equipaje"
...
export default ResponsiveDataEditor;